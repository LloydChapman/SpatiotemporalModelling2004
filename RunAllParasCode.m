...
rslts=cellfun(@(x)['MCMC_' IPD x],Mdls,'UniformOutput',false);
u=[{3,[1,3]},repmat({1:3,1:4},1,8)];
typ=[{'Const','Const'},repmat({'Cauchy','Cauchy','Exp','Exp'},1,2),{'Exp','Exp','Exp','Exp'},{'Cauchy','Cauchy','Exp','Exp'}];
beta0s=[0,beta0*ones(1,nMdls-1)];
alpha0s=[0,0,alpha0*ones(1,nMdls-2)];
...
%% Run MCMC
RunVLStmMCMC(db,r,a,b,u,beta0s,alpha0s,epsilon0,delta0s,h0s,typ,inclLST,niters,plotOutpt,rslts)
...
%% Calculate DIC
RunCalcDIC(rslts(1:14),burnin,str)
